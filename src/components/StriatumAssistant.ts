...
function answer(raw: string): string {
  ...
  if (/date|when|schedule|programme|program/.test(query)) {
    ...
    return `<p>The conclave programme runs <b>14–18 OCT 2026</b>. Here are the dated activity windows:</p><div class="s4-assistant-dates">${dates}</div>`;
  }
  if (/delegate|pass|aqualume|synexa|register/.test(query)) return `<p>A Delegate Pass unlocks the delegate-only workshops and conclave access. Start your application here:</p><a class="s4-assistant-link" href="/delegate">Open delegate registration <b>↗</b></a>`;
  ...
  if (matches.length && terms.some(term => topicAliases[term])) {
    ...
    return `<p>Here are the conclave activities related to <b>${esc(topic)}</b>:</p>${matches.slice(0, 8).map(eventLine).join('')}`;
  }
  ...
  return `<p>I couldn’t match that to the confirmed conclave catalogue. Try an event name, specialty, “dates”, “delegate pass”, or “explore”.</p><a class="s4-assistant-link" href="/explore">Browse all activities <b>↗</b></a>`;
}
...
export function mountStriatumAssistant(): void {
  ...
  root.innerHTML = `<button class="s4-assistant-orb" aria-label="Open STRIATUM field guide"><span class="s4-assistant-avatar" aria-hidden="true"></span></button>
    <section class="s4-assistant-panel" aria-label="STRIATUM event assistant">
      ...
      <div class="s4-assistant-feed"><div class="s4-assistant-message"><small>INDEX / 4.0</small><p>I can navigate the conclave catalogue for you. What are you looking for?</p></div></div>
