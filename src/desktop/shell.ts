...
const PATHS: Record<IconName, string> = {
  ...
  // Two events converging into one bundle — the bead-and-rail motif, not a
  // shopping glyph.
  combos: '<path d="M8.4 8.2 15 11"/><path d="M8.4 15.8 15 13"/><circle cx="5.5" cy="7" r="2.4"/><circle cx="5.5" cy="17" r="2.4"/><circle cx="17.6" cy="12" r="2.6"/>',
...
const RAIL_TABS: { id: ScreenType; icon: IconName; label: string }[] = [
  ...
  { id: 'combos', icon: 'combos', label: 'COMBOS' },
...
function railSectionFor(screen: ScreenType): ScreenType | null {
  switch (screen) {
    ...
    case 'profile':
      return screen;
    // My Events has left the primary navigation; it is reached from Profile,
    // so highlight Profile while it is open rather than nothing at all.
    case 'my-events':
      return 'profile';
    default:
      return null;
