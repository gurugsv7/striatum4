-- ============================================================================
-- Senior quiz and case-presentation fees, as published by the organisers.
--
-- AQUAQUEST is 400 a head and 700 a team. The engine carries either phase fees
-- (entry/spot) or per-person/per-team fees, never both, so the spot prices
-- (450 and 850) are published on the event page as facts rather than modelled;
-- the site charges entry prices, and spot registration is taken at the desk.
-- Its participation becomes 'either', which is what it now is.
--
-- THE DIAGNOSTIC ABYSS is a flat 400, payable by those selected for oral
-- presentation, replacing the individual 300 / team 400 split.
--
-- Two guards in create_order had to follow:
--
--   * participation was forced to 'team' for any team event, which would have
--     charged a lone AQUAQUEST entrant the 700 team fee. The roster decides
--     now in every case. A roster of one is only possible where min_members
--     allows one, so no team-only event is affected.
--
--   * buying several team entries at once was allowed only for participation
--     = 'team', which would have broken combo-aquaquest-4-teams the moment
--     AQUAQUEST became 'either'. The test is now whether the event has teams
--     at all.
--
-- Verified against the live database:
--   AQUAQUEST, one person, request claiming "team"    -> 400, basis Individual
--   AQUAQUEST, three people, claiming "individual"    -> 700, basis Team
--   AQUAQUEST four-team combo pack                    -> 2500, four Team lines
--   THE DIAGNOSTIC ABYSS, two people                  -> 400, basis Registration
-- (probe orders removed afterwards)
-- ============================================================================

update public.events
   set participation = 'either',
       price_entry = null,
       price_spot = null,
       price_individual = 400,
       price_team = 700,
       price_unit = null
 where id = 's4-12';

update public.events
   set price_individual = null,
       price_team = null,
       price_flat = 400,
       price_unit = null
 where id = 's4-15';
