-- ============================================================================
-- The fee basis is decided by the roster, not by the browser.
--
-- Three events (THE DIAGNOSTIC ABYSS, CORAL CANVAS, CHIRONEX) are priced
-- individual 300 / team 400 and accept one or two people. create_order took
-- `participation` straight from the request and priced on it, while
-- validate_roster only ever checked the roster against min/max members. The two
-- were never compared, so a request could name two participants and still claim
-- 'individual'.
--
-- Verified against the live database before this change: a two-person CHIRONEX
-- roster sent as 'individual' was accepted at 300 instead of 400 (probe order
-- removed afterwards).
--
-- Whenever a roster is supplied it now decides: a team entry naming two or more
-- people is a team, one person is an individual, and a team-only event stays a
-- team however it is asked for. Without a roster the previous behaviour stands,
-- which is what combo and quantity-only lines rely on.
--
